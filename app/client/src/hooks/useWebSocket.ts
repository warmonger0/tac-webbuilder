...
import type { Workflow, Route, WorkflowHistoryItem, WorkflowHistoryUpdateMessage } from '../types';
...
export function useWorkflowHistoryWebSocket() {
  ...
  const { data: polledHistory } = useQuery({
    ...
    queryFn: () => getWorkflowHistory({ sort_by: 'date', order: 'desc', limit: 50 }),
    refetchInterval: isConnected ? false : 3000, // Only poll when WS disconnected
    enabled: !isConnected, // Only enable when WS is not connected
  });
  ...
  useEffect(() => {
    if (!isConnected && polledHistory) {
      setWorkflowHistory(polledHistory.items);
      setLastUpdated(new Date());
    }
  ...
  useEffect(() => {
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        try {
          const message: WorkflowHistoryUpdateMessage = JSON.parse(event.data);

          if (message.type === 'workflow_history_update') {
            setWorkflowHistory(message.data);
            setLastUpdated(new Date());
            console.log('[WS] Received workflow history update:', message.data.length, 'items');
          }
        } catch (err) {
          console.error('[WS] Error parsing message:', err);
        }
  ...
  return { workflowHistory, isConnected, lastUpdated };
}
