  ...
  return (
    <div className="mt-2 space-y-4">
      ...
      <WorkflowProgressVisualization
        currentPhase={(state as any).current_phase}
        phaseProgress={(state as any).phase_progress}
        startTime={(state as any).start_time || state.start_time}
        endTime={(state as any).end_time}
        status={state.status}
        errorCount={(state as any).error_count}
        lastError={(state as any).last_error}
      />
