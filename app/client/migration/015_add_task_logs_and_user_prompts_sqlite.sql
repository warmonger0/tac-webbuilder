...
-- Note: SQLite doesn't support CREATE OR REPLACE VIEW, so we use DROP/CREATE pattern

DROP VIEW IF EXISTS v_task_logs_latest;
CREATE VIEW v_task_logs_latest AS
SELECT
  issue_number,
  adw_id,
  phase_name,
...
DROP VIEW IF EXISTS v_user_prompts_with_progress;
CREATE VIEW v_user_prompts_with_progress AS
SELECT
  up.*,
  ip.total_phases,
  ip.completed_phases,
