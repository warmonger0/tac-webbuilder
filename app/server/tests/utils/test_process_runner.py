...
class TestProcessRunnerRun:
    ...
    def test_run_with_log_command_true(self, mock_subprocess_success):
        ...
        """
        with patch("subprocess.run", return_value=mock_subprocess_success), patch("utils.process_runner.logger") as mock_logger:
    ...
    def test_run_logs_timeout(self):
        ...
        with patch("subprocess.run", side_effect=timeout_exception), patch("utils.process_runner.logger") as mock_logger:
    ...
    def test_run_logs_error(self):
        ...
        with patch("subprocess.run", side_effect=error), patch("utils.process_runner.logger") as mock_logger:
...
class TestProcessRunnerIntegration:
    ...
    def test_full_success_flow(self):
        ...
        with patch("subprocess.run", return_value=mock_result), patch("utils.process_runner.logger"):
            result = ProcessRunner.run(
                ["python", "-m", "pytest"],
                timeout=30,
                capture_output=True,
