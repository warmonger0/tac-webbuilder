...
def test_scan_agents_directory_with_workflows(temp_db):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(Path, 'parent', agents_dir.parent), patch('core.workflow_history_utils.filesystem.Path.__truediv__', return_value=agents_dir):
            workflows = scan_agents_directory()
...
def test_sync_workflow_history(temp_db):
    ...
    with patch('core.workflow_history_utils.sync_manager.scan_agents_directory', return_value=mock_workflows), patch('core.workflow_history_utils.enrichment.read_cost_history', side_effect=Exception("No cost data")):
...
def test_cost_sync_completed_workflow_updates_final_cost(temp_db):
    ...
    with patch('core.workflow_history_utils.sync_manager.scan_agents_directory', return_value=mock_workflows), patch('core.workflow_history_utils.enrichment.read_cost_history', return_value=mock_cost_data):
        synced = sync_workflow_history()
        assert synced >= 1
...
def test_cost_sync_running_workflow_prevents_decreases(temp_db):
    ...
    with patch('core.workflow_history_utils.sync_manager.scan_agents_directory', return_value=mock_workflows), patch('core.workflow_history_utils.enrichment.read_cost_history', side_effect=Exception("No cost")):
        sync_workflow_history()
