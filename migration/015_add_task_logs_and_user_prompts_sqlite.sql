-- Migration 015: Add task_logs and user_prompts tables (SQLite version)
-- Created: 2025-12-02
...
CREATE TABLE IF NOT EXISTS user_prompts (
  ...
  project_path TEXT,
  auto_post INTEGER DEFAULT 0, -- SQLite uses INTEGER for boolean
  ...
  -- Multi-Phase Info
  is_multi_phase INTEGER DEFAULT 0, -- SQLite uses INTEGER for boolean
...
CREATE VIEW IF NOT EXISTS v_task_logs_latest AS
SELECT
  issue_number,
  adw_id,
  phase_name,
...
CREATE VIEW IF NOT EXISTS v_issue_progress AS
SELECT
  issue_number,
  adw_id,
  workflow_template,
...
CREATE VIEW IF NOT EXISTS v_user_prompts_with_progress AS
SELECT
  up.*,
  ip.total_phases,
  ip.completed_phases,
